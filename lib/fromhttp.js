...
function fromHttp(app, queueImpl, opts, ttl) {
    ...
    function version2(parsed, continuations) {
        ...
        var durationMs = (new Date()).getTime() - reqTime;
        ...
        if (parsed.end) {
            ...
            var metric = {
                ...
                len: continuation.bodyLength,
                txid: correlationId,
                pDuration: durationMs,
                sDuration: parsed.duration,
                node: parsed.headers['x-node'],
                ns: cc.proxy.request
            };
            console.log("METRIC " + _.map(metric, function(val,key) { return key +'="'+val + '"'}).join(" "));
