...
function fromHttp(app, queueImpl, opts, ttl) {
    ...
    subscriber.on('message', handleMessage); // Received response message from queue
	...
	if (parsed.version) {
	    sendHttpResponse(parsed);
	} else if (parsed.ack) {
	    if (continuations[parsed.correlationId])
		continuations[parsed.correlationId].ack = parsed.ack;
	} else {
	    console.error('message format not supported');
	}
    }

    function sendHttpResponse(parsed) {
	if (parsed.version) {
    ...
    function version2(parsed, continuations) {
        ...
        if (parsed.end) {
            ...
            //console.log(req.method, 2, parsed.code, req.url, continuation.bodyLength, correlationId,  durationMs, parsed.headers['x-node'], continuation.bodySample);
        ...
        function forwardRequest(destination) {
            ...
            setTimeout(function () {
                if (continuations[correlationId]) {
                    ...
                    var req = c.req;
                    console.error("******************* Continuation expired for "
				  + req.method + ", " + req.url + " (" + correlationId + ")"
				  + (c.ack ? ", Last ACK: " + JSON.stringify(c.ack) : '' )
				  + ', Destination', destination);
                    delete continuations[correlationId];
